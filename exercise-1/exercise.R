...
pub_repo_num <- sum(nrow(body_data))
...
query <- GET(paste0(base_uri, resource), query = query_params)

# Extract the response body and convert it from JSON.
response_content <- content(response, "trxt")
...
repo_names <- results$items$full_name[1:5]
print(repo_names)
